...
import { FaCog, FaUserAlt, FaChevronDown, FaChevronUp, FaCalendarCheck, FaWallet, FaBars, FaPlus } from "react-icons/fa";
          ...
          {menuItems.map((item) => (
            <li key={item.name} className="text-black">
              <div
                ...
                onClick={() => {
                  item.route ? handleNavigation(item.route) : handleDropdownToggle(item.name)
                }
              ...
              {item.items && activeDropdown === item.name && (
                <ul className="pl-8 mt-2 space-y-2">
                  {item.items.map((subItem) => (
                    <li
                      key={subItem.name}
                      className="text-sm rounded p-1 transition-colors cursor-pointer duration-200 hover:bg-gray-200 hover:text-gray-800"
